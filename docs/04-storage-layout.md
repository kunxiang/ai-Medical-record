...
| `capture.json` / `page-NN.json` / `audio/*.json` | L1 事实 | 写后永不改 | ✅ 同上 | 必带 | 必带 |
...
事件类型注册表在 `_meta/README.md` 维护。覆盖范围(设计债 D1 的完整清单):问答**全部**答案类型(点选/数字/文字/日期/照片/语音)、手动 observation(血压/体重)、监控组定义、observation 修正与确认、encounter 编辑、归人裁决及备注、决策状态变更引用。
...
- 缩略图(≤ 400px)与预览图(≤ 1600px)在服务端生成,放 `derived/`。
