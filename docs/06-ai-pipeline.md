...
- **派生物按人工确认的角点裁边(ADR-052)。** 生产供应商把每张图归一到固定的 384 图像 token,
  尺寸买不到更多像素 —— 像素预算是死的,画面里给桌面和背景的每一个像素都是从单据文字身上抢走的。
  裁掉背景是唯一还剩的提高**每字符像素数**的杠杆,顺带缩小跨境回拉的体积。角点在采集端由检测器
  提出、由人在上传前确认或修正,存进 `capture.json`;渲染顺序恒为 `.rotate()` → 裁切 → `.resize()`
  —— 裁切必须在全分辨率上做,放到 resize 之后等于白做。裁切**只作用于 `ai` 变体**:`thumb`/`preview`
  的消费者是人眼,复核时要拿它跟原件比对,裁过的预览会让人核对的是被处理过的东西。
...
**full_text 分两层**(消解与 ADR-015「完整性」的矛盾,审核 C-F8):S3 工件存**带 PII span 标注的完整文本**(落 `derived/.../extractions/`,与原件同**保留策略**但归属 L2 可重跑,不出库;ADR-045);入全文索引与 embedding 前,由**确定性代码**按 span 遮蔽。回归断言相应改为:**索引层出现手机号即判定失败**(而非提取输出层 —— 提取层必须完整)。
