...
// 除本文件外任何 src 下出现 `claude-*` / `deepseek-v*` 模型字面量都判失败。
//
...
if ((AI_PROVIDER === 'deepseek' && !MODEL.startsWith('deepseek-'))
  || (AI_PROVIDER === 'anthropic' && !MODEL.startsWith('claude-'))) {
  throw new Error(`AI_MODEL 与 AI_PROVIDER=${AI_PROVIDER} 不匹配`);
}
