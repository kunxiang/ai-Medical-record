...
| C2 | ✅ 2026-08-26 项目所有者确认 M0 可验收；本项不再作为关闭门禁。仓库未新增独立执行记录。 |
| C3 | ✅ 2026-08-26 项目所有者确认 M0 可验收；本项不再作为关闭门禁。仓库未新增独立执行记录。 |
...
A + B 全绿,C1 有执行证据；2026-08-26 项目所有者确认接受剩余人工风险并验收，**M0 关闭**。

## 2026-08-26 补录：家庭成员创建可达性

复核发现 M0 的 `POST /api/v1/people`、授权、`_person.json` 与 journal 原子建档链均已交付并有 A4 证据，但 PWA 在 M1 收口时没有接入创建入口。后端 M0 结论不变；跨里程碑的前端可达性缺口转入 M1 补录。
