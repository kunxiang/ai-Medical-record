...
| # | 结果 |
|---|---|
| C1 | ✅ 2026-08-26 项目所有者确认 M1 可验收；本项不再作为关闭门禁。仓库未新增独立真机执行记录。 |
| C2 | ✅ 2026-08-26 项目所有者确认 M1 可验收；本项不再作为关闭门禁。仓库未新增独立真机执行记录。 |
| C3 | ✅ 2026-08-26 项目所有者确认 M1 可验收；本项不再作为关闭门禁。仓库未新增独立真实单据执行记录。 |
...
A(88/88)+ B(12 项)全绿,D11/D12 已勾销；2026-08-26 项目所有者确认接受剩余人工风险并验收，**M1 关闭**。

## 2026-08-26 验收补录：家庭成员创建

项目所有者在关闭后发现人员选择器只有切换能力，无法从 PWA 创建家庭成员。根因是 M0 建档 API 未接入 M1 前端，而原 A1 只用 API 预建人员后验证选择器，因此没有覆盖用户建档路径。

- 已补：采集页“添加成员”入口、必填校验、失败提示、创建后自动切换，以及只含四个允许字段的 `people_cache` 更新。
- 已补验收项：`m1-99 A1b`，覆盖 UI 提交、201、新成员渲染、自动选择和精简缓存边界。
- 当前证据：web 与 tools TypeScript 检查、web 生产构建均通过；使用真实生产组件与拦截 API 的 Chromium 冒烟在桌面 1440×900、手机 390×844 均通过创建、自动切换、顶栏同步、精简缓存和无控制台错误。A1b 尚未随完整 M1 破坏性验收编排复跑，项目所有者界面预览尚待确认。

因此原 88/88 结果仍是当时已执行范围的真实记录，但不能作为本补录功能的通过证据；M1 状态暂记为**补录待验收**。
