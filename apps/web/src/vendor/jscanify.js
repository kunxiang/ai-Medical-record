...
/* eslint-disable */
// 内联自 npm 包 jscanify@1.4.3 的 `jscanify/client` 入口(浏览器版,原文件 7 KB、零 import)。
//
// 为什么内联而不是依赖:jscanify 的默认入口是 Node 版,package.json 里挂着 `canvas`(原生编译)
// 与 `jsdom`。我们只用浏览器那个文件,却会让整个工作区在每次 CI 安装时背上一套原生构建工具链。
// 上一版 UMD 包装依赖全局赋值,这里改成 ESM 默认导出;算法本体一字未改。
//
// 它需要一个全局 `cv`(OpenCV.js),由 offline/crop.ts 在调用前注入。
// 这份代码是可抛弃的:我们存的是角点不是图,换检测器不影响任何已存的人工确认结果。
